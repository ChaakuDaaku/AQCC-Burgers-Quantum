...
class Circuit:
    ...
    def _maybe_batch(self, param, convert_to_tensor=True):
        ...
        if convert_to_tensor:
            p = tf.convert_to_tensor(value=param)
    ...
    def prepare_multimode(self, state, modes, input_state_is_pure=False):
        ...
        if self._valid_modes(modes):
            ...
            with self._graph.as_default():
                state = tf.cast(tf.convert_to_tensor(value=state), ops.def_type)
        ...
        with self.graph.as_default():
            ...
            if select is not None:
                ...
                if self._state_is_pure and len(modes) == self._num_modes:
                    ...
                    sample = tf.random.categorical(logits=tf.reshape(logprobs, [batch_size, -1]), num_samples=1)
                    ...
                    if num_reduced_state_modes > 1:
                        ...
                        reduced_state_reshuffled = tf.transpose(a=reduced_state, perm=transpose_list)
                    else:
            ...
            if len(modes) == self._num_modes:
                ...
                if self._state_is_pure:
                    norm = tf.norm(tensor=tf.reshape(conditional_state, [batch_size, -1]), axis=1)
                else:
    ...
    def measure_homodyne(self, phi, mode, select=None, **kwargs):
        ...
        with self.graph.as_default():
            ...
            if select is not None:
                ...
                rho_dist = tf.cast(tf.reduce_sum(input_tensor=hermite_terms, axis=[1, 2]), tf.float64) \
                ...
                samples_idx = tf.random.categorical(logits=logprobs, num_samples=1)
            ...
            if self._num_modes == 1:
                ...
                # only some modes were measured: put unmeasured modes in conditional state, while reseting measured modes to vac
                inf_squeezed_vac = tf.convert_to_tensor(value=[(-0.5) ** (m // 2) * np.sqrt(factorial(m)) / factorial(m // 2) if m % 2 == 0 else 0. for m in range(self._cutoff_dim)],
                                                        dtype=ops.def_type)
                if self._batched:
                    inf_squeezed_vac = tf.tile(tf.expand_dims(inf_squeezed_vac, 0), [batch_size, 1])
                displacement_size = tf.stack(tf.convert_to_tensor(value=meas_result * np.sqrt(m_omega_over_hbar / 2)))
