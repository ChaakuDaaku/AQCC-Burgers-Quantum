...
def layer(q):
    ...
    theta_variables_1 = tf.Variable(tf.random.normal(shape=[BS_variable_number]))
    phi_variables_1 = tf.Variable(tf.random.normal(shape=[BS_variable_number]))
    rphi_variables_1 = tf.Variable(tf.random.normal(shape=[R_variable_number]))

    theta_variables_2 = tf.Variable(tf.random.normal(shape=[BS_variable_number]))
    phi_variables_2 = tf.Variable(tf.random.normal(shape=[BS_variable_number]))
    rphi_variables_2 = tf.Variable(tf.random.normal(shape=[R_variable_number]))

    s_variables = tf.Variable(tf.random.normal(shape=[N], stddev=0.0001))
    d_variables_r = tf.Variable(tf.random.normal(shape=[N], stddev=0.0001))
    d_variables_phi = tf.Variable(tf.random.normal(shape=[N]))
    k_variables = tf.Variable(tf.random.normal(shape=[N], stddev=0.0001))
...
difference = tf.reduce_sum(input_tensor=tf.abs(ket - target_state))
fidelity = tf.abs(tf.reduce_sum(input_tensor=tf.math.conj(ket) * target_state)) ** 2
cost = difference
